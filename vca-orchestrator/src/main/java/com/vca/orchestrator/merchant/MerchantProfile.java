...
/**
 * 一家商家(诊所/机构)的完整资料 —— 电话客服按它"知道自己是谁、卖什么、几点开门"。
 *
 * <p><b>为什么要结构化字段, 而不是全靠上传文档做向量检索</b>: 营业时间、地址、价格、医生这类问题占诊所
 * 来电的大头, 向量检索有召回失败的风险, 还要多一次接口调用; 而线上真出过 AI 编造"5 家门店"的事,
 * 根子就是这类信息没有确定的来源。做成字段, 每通电话直接拼进提示词, 又准又快。上传文档只兜长尾。
 *
 * <p>与 {@code vca-telephony} 里的 {@code Merchant} 的关系: 这是<b>持久化的资料</b>(诊所在网页上填的东西),
 * 那是<b>一通电话运行时用的配置</b>(已经把资料渲染进人设、把留空的项回退成全局默认)。前者归诊所维护,
 * 后者由注册表按需生成。
 *
 ...
 * @param services           项目与价格(自由文本, 一行一项)
 * @param doctors            医生/老师(自由文本)
 * @param bookingRules       预约规则(要留什么信息、提前多久、取消改期怎么办)
...
public record MerchantProfile(
    ...
    /**
     * 把结构化资料渲染成一段给模型看的文本。只输出填了的项, 什么都没填返回空串。
     *
    ...
    public String renderProfile() {
        ...
        line(sb, "机构名称", name);
        line(sb, "地址", address);
        line(sb, "营业时间", businessHours);
        line(sb, "对外电话", phone);
        line(sb, "交通与停车", transport);
        block(sb, "项目与价格", services);
        block(sb, "医生/老师", doctors);
        block(sb, "预约规则", bookingRules);
        block(sb, "其他说明", notes);
        if (sb.isEmpty()) {
            return "";
        }
        return "以下是本机构的资料, 回答客户时以此为准。资料里没有的信息(价格、时间、地址、人员等)一律不要编造,"
                + " 如实说不清楚并提出帮客户转给同事或回电确认。\n" + sb;
    }
