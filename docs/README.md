...
| [10 · 电话接入](./10-telephony-outbound.md) | 用同一套对话内核接电话线路（默认 FreeSWITCH，备选 Asterisk）：呼入呼出、媒体转码、电话专用 VAD |
| [11 · 声音复刻](./11-voice-clone.md) | **用自己的声音说话 + 方言**：采集为何自编 WAV、样本不落 OSS（实测）、音色→模型前缀路由、归属校验（含一个被测试逼出的越权）、指令控制说方言 |
| [12 · FreeSWITCH 接入](./12-freeswitch.md) | **电话怎么接进对话引擎**：拨号计划 `socket` 应用 + 内核 `unicast` 双向音频（零第三方模块）、呼入/外呼完整过程、握手协议、配置表、Linphone 实测踩坑与排查 |
| [13 · 运营后台与商家后台](./13-console.md) | **两个网页后台**：运营在页面上添加商家、开通网关（不用服务器命令行）、看全局来电；商家自己维护门店资料、知识库、看通话与线索 |
