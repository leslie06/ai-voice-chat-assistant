...
**开通一家店的网关**：日常在运营后台的页面上开通（见 [13 · 运营后台与商家后台](./13-console.md)），不用上服务器。
下面的脚本留作后台不可用时的备用，在服务器上执行
...
群推送会被刷掉、没配推送地址的店干脆收不到，所以网页上也要能查。门店所属账号在商家后台（`/merchant/`）、
运营管理员在运营后台（`/admin/`）里可以看（见 [13 · 运营后台与商家后台](./13-console.md)）：
...
网页端的表单在商家后台（`/merchant/`）与运营后台（`/admin/`）里，见 [13](./13-console.md)；语音助手首页的设置里不再有门店表单。
新加载到一家商家时会预合成它的开场白
